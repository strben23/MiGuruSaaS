...
const handleLogin = async () => {
  ...
  if (!username.value || !correo.value) {
  ...
  const res = await fetch('http://localhost:3001/api/usuarios/login', {
    ...
    body: JSON.stringify({ nombre: username.value, correo: correo.value }),
  })
  ...
  if (res.ok) {
    successMessage.value = `Bienvenido, ${data.usuario.nombre}!`
    // ✅ Redirect to /dashboard after success
      setTimeout(() => {
        router.push('/dashboard')
      }, 1000) // Optional: short delay so the success message is visible
