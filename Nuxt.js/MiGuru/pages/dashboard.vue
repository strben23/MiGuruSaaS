...
const fetchClasses = async () => {
  ...
  try {
    ...
    errorLoading.value = error.data?.error || error.message || "No se pudieron cargar las clases. Inténtalo de nuevo más tarde.";
    classes.value = [];
  } finally {
...
<template>
  <div class="p-4 sm:p-6 md:p-8">
    <div class="flex justify-between items-center mb-8 p-4 sm:p-6 bg-slate-800 rounded-lg shadow-md border border-slate-700">
      <h1 class="text-3xl font-bold text-sky-400">
        Mis Cursos
      </h1>
      <NuxtLink
        to="/add-class"
        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-sky-500 transition-colors"
      >
    ...
    <div v-else-if="errorLoading" class="mt-6 p-4 bg-red-500/20 border border-red-500/50 rounded-lg text-red-300 text-sm text-center">
      ...
      <p>{{ errorLoading }}</p>
      <button @click="fetchClasses" class="mt-3 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-md transition-colors">
        Reintentar
      </button>
    </div>
    ...
    <div v-else-if="noClassesMessage" class="text-center py-20">
      ...
      <NuxtLink
        to="/add-class"
        class="mt-6 inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-sky-500 transition-colors"
      >
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      >
        <ClassCard
          :classItem="classItem"
          class="bg-slate-800 border border-slate-700 rounded-lg shadow-lg hover:shadow-sky-500/30 transition-all duration-300 ease-in-out overflow-hidden transform group-hover:scale-105 group-hover:border-sky-600 h-full flex flex-col"
        />
      </NuxtLink>
    </div>
  </div>
